...
def num_splits(s, d):
    ...
    return helper(s, 0) // 2
    '''for every number in s, two choices: + or -, and when you reach the end of s(if not s), you have complete the 
    calculation(split s into two parts and '-' them), then compare the difference with d'''
...
def reverse(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    return foldl(lst, lambda f, r: Link(r, f), Link.empty)
