...
def balanced(m):
    ...
    if is_planet(m):
        return True
    wl = total_weight(end(left(m)))
    wr = total_weight(end(right(m)))
    if length(left(m)) * wl == length(right(m)) * wr:
...
def multiple_references_explanation():
    ...
    the same interval more than once."""# official-explanation
...
def quadratic(x, a, b, c):
    ...
    def cal(m):
        return a * m * m + b * m + c
    extreme = cal(p)
    p1 = cal(lower_bound(x))
    p2 = cal(upper_bound(x))
    if lower_bound(x) <= p <= upper_bound(x):
        return interval(min(extreme, p1, p2), max(extreme, p1, p2))
    else:
