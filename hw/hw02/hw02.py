...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def pp(i, result, diff):
        if i == n:
            return result
        if num_eights(i) or i % 8 == 0:
            return pp(i + 1, result - diff, -diff)
        else:
            return pp(i + 1, result + diff, diff)
    return pp(1, 1, 1)
    '''if n == 1:
...
def count_coins(total):
    ...
    def cc(total, min):
        ...
        if min == None:
            ...
            with_c = cc(total, next_largest_coin(min))
            without_c = cc(total - min, min)
...
def make_anonymous_factorial():
    ...
    True
    """
    return (lambda x:x(x))(lambda f: lambda x: 1 if x == 0 else x * f(f)(x - 1))
