...
class VendingMachine:
    ...
    def restock(self, num):
        self.inventory += num
        return "Current {0} stock: {1}".format(self.ele, self.inventory)
    ...
    def vend(self):
        if self.inventory == 0:
            ...
            self.inventory -= 1
            if diff:
                return "Here is your {0} and ${1} change.".format(self.ele, diff)
            else:
                return "Here is your {}.".format(self.ele)
...
def store_digits(n):
    ...
    while t > 10:
        t //= 10
        c += 1
    return Link(t, store_digits(n % (10 ** c)))
    ''' 
    result = Link.empty
    while n > 0:
...
def is_bst(t):
    ...
    if len(t.branches) == 2:
        if t.branches[0].label > t.label or t.branches[1].label <= t.label:
            return False
        mi = bst_min(t.branches[1])
        ma = bst_max(t.branches[0])
        if t.label < ma or t.label > mi:
            return False
    for b in t.branches:
        if not is_bst(b):
            return False
