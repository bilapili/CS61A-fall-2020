...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 0:
            cur = score0
            inc = score0 - last_score
            if inc > running_high:
                print(inc, "point(s)!", "The most yet for Player 0")
                high = inc
            else:
                high = running_high
        elif who == 1:
            cur = score1
            inc = score1 - last_score
            if inc > running_high:
                print(inc, "point(s)!", "The most yet for Player 1")
                high = inc
            else:
                high = running_high
        return announce_highest(who, cur, high)
    return say
    # END PROBLEM 7
...
def final_strategy(score, opponent_score):
    ...
    If you are taking the lead and your distance to the goal is less than 5,
    roll 0, 1, 2 to reduce the risks.
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    if score + free_bacon(opponent_score) >= 100:
        return 0
    if score + roll_dice(1) >= 100:
        return 1
    if score + roll_dice(2) >= 100:
        return 2
