...
def accuracy(typed, reference):
    ...
    for t in typed_words:
        if index == len(reference_words) or index == len(typed_words):
            break
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    min_diff, index = 100, 0
    for w in range(len(valid_words)):
        diff = diff_function(user_word, valid_words[w], limit)
        if diff < min_diff:
            min_diff = diff
            index = w
    if min_diff > limit:
    ...
    ''' if user_word in valid_words:
        ...
        if diff_function(possible_word, user_word, limit) > limit:
            ...
            return possible_word''' #better solution.
    # END PROBLEM 5
...
def report_progress(typed, prompt, user_id, send):
    ...
    "*** YOUR CODE HERE ***"
    correct = 0
    while correct < min(len(typed), len(prompt)) and typed[correct] == prompt[correct]:
        correct += 1
...
def time_per_word(times_per_player, words):
    ...
    for t in times_per_player:
        times += [[t[i + 1] - t[i] for i in range(len(words))]]
...
def fastest_words(game):
    ...
    for w in word_indices:
        ...
        for p in player_indices:
            if time(game, p, w) < fast:
                fast = time(game, p, w)
        for p in player_indices:
            if time(game, p, w) == fast:
                fastest[p].append(word_at(game, w))
                break
    return fastest
    ''' 
