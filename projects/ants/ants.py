...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        range = 0
        current = self.place
        while range < self.min_range and current is not beehive:
            current = current.entrance
            range += 1
        while range < self.max_range and current is not beehive:
            if len(current.bees) > 0:
                break
            current = current.entrance
            range += 1
        if current is beehive:
            return None
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        while place is not None:
            if place.ant and not place.ant.buff:
                place.ant.damage *= 2
                place.ant.buff = True
            place = place.exit
        # END Problem EC
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        Ant.reduce_armor(self, amount)
        if self.armor <= 0 and self.true == 0:
            bees_win()
        # END Problem EC
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        # BEGIN Problem Optional 5
        ins, cur_p, dis = {}, self.place, 0
        while cur_p is not beehive:
            if cur_p.ant and not isinstance(cur_p.ant, LaserAnt):
                ins[cur_p.ant] = dis
            for b in cur_p.bees:
                ins[b] = dis
            cur_p = cur_p.entrance
            dis += 1
        return ins
        # END Problem Optional 5
